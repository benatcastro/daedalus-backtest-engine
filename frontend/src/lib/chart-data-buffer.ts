...
export class ChartDataBuffer<T extends TimeBasedData> {
  ...
  constructor(
    ...
    initialData: T[],
    bounds: DataBounds,
    config: BufferConfig = { maxSize: 5000, chunkSizeHours: 24, threshold: 10, trimSize: 1000 }
  ) {
    ...
    console.log("Data Buffer created, initial data length:", initialData.length)
    console.log("Initial view range:", startingViewRange)
    console.log("Data bounds:", bounds)
    this.logDateRange(startingViewRange.from, startingViewRange.to, "Initial view range")
  ...
  public getDataRange(): IRange<Time>  {
    ...
    const sortedData = [...this._data].sort((a, b) => (a.time as number) - (b.time as number));
  ...
  logDateRange(from: Time, to: Time, message: string): void {
    ...
    console.log(`${message}: ${green}${fromDate} -> ${toDate}${reset} || ${from} -> ${to}`);
  }
  ...
  public async updateData(timeMovement: Time) {
    ...
    try {
      if (this._data.length === 0) {
        ...
        const requestedRange = timeMovement as number > 0 ? {
          from: (this._currentRange.from as number) as Time,
          to: (this._currentRange.to as number + (timeMovement as number * this._config.threshold)) as Time
        } : {
          from: (this._currentRange.from as number + (Math.abs(timeMovement as number) * this._config.threshold)) as Time,
          to: (this._currentRange.to as number + this._config.threshold) as Time
        };

        const constrainedRange = this.constrainToBounds(requestedRange);

        this._data = await this._dataFetcher.fetchData(constrainedRange.from, constrainedRange.to);
        console.log(`Fetched initial data. Bounds: ${this._data[0]?.time} -> ${this._data[this._data.length - 1]?.time}`);
        return;
      }

      const currentDataRange = this._dataRange;
      const requestedRange = {
        from: (this._currentRange.from as number - this._config.threshold) as Time,
        to: (this._currentRange.to as number + this._config.threshold) as Time
      };
      ...
      if ((constrainedRange.from as number) < (currentDataRange.from as number)) {
        // Only fetch if we're not already at the start bound
        if ((currentDataRange.from as number) > (this._bounds.start as number)) {
          console.log(`${blue}Prepending data from ${constrainedRange.from} to ${currentDataRange.from}${reset}`);
          const prependData = await this._dataFetcher.fetchData(
            constrainedRange.from,
            currentDataRange.from
          );
          this._data = this.mergeDatWithoutDuplicates(this._data, prependData, 'prepend');
          console.log(`Prepended ${prependData.length} items. Total: ${this._data.length}`);
        } else {
          console.log(`${blue}Already at start bound, skipping prepend${reset}`);
        }
      }
      ...
      if ((constrainedRange.to as number) > (currentDataRange.to as number)) {
        ...
        if ((currentDataRange.to as number) < (this._bounds.end as number)) {
          console.log(`${blue}Appending data from ${currentDataRange.to} to ${constrainedRange.to}${reset}`);
          const appendData = await this._dataFetcher.fetchData(
            currentDataRange.to,
            constrainedRange.to
          );
          this._data = this.mergeDatWithoutDuplicates(this._data, appendData, 'append');
          console.log(`Appended ${appendData.length} items. Total: ${this._data.length}`);
        } else {
      ...
      console.log(`Updated data bounds: ${this._data[0].time} -> ${this._data[this._data.length - 1].time}`);
    } finally {
  ...
  private mergeDatWithoutDuplicates(existingData: T[], newData: T[], operation: 'prepend' | 'append'): T[] {
    const combinedData = operation === 'prepend' ? [...newData, ...existingData] : [...existingData, ...newData];

    // Remove duplicates based on time and sort chronologically
    const uniqueData = combinedData.reduce((acc: T[], current: T) => {
      const existingIndex = acc.findIndex(item => (item.time as number) === (current.time as number));
      if (existingIndex === -1) {
        acc.push(current);
      } else {
  ...
  private hasDataForRange(range: IRange<Time>): boolean {
    ...
    return (currentDataRange.from as number) <= (range.from as number) &&
  ...
  private constrainToBounds(range: IRange<Time>): IRange<Time> {
    return {
      from: Math.max(range.from as number, this._bounds.start as number) as Time,
