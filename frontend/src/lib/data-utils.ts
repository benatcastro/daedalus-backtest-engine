...
export function generatePaginatedCandlestickData(
    ...
    const chunkEndMs = Math.min(chunkStartMs + chunkDurationMs, fullBacktestRange.end.getTime());
    ...
    const limitedData = data.length > maxCandlesPerChunk ? data.slice(0, maxCandlesPerChunk) : data;
...
export function calculateOptimalChunkSize(
    ...
    const totalHours =
        (backtestRange.end.getTime() - backtestRange.start.getTime()) / (1000 * 60 * 60);
...
export function calculateOptimalInitialViewRange(
    ...
    const totalDurationMs = backtestRange.to.getTime() - backtestRange.from.getTime();
    ...
    const startTime = Math.max(endTime - actualDurationMs, backtestRange.from.getTime());
