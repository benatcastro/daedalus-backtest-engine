import { CandlestickData, IRange, Time } from "lightweight-charts";
...
export function generatePaginatedCandlestickData(
  ...
  const totalDurationMs =
    fullBacktestRange.end.getTime() - fullBacktestRange.start.getTime();
  ...
  const chunkStartMs =
    fullBacktestRange.start.getTime() + chunkIndex * chunkDurationMs;
  ...
  const actualStartMs =
    chunkIndex > 0 ? chunkStartMs - overlapMs : chunkStartMs;
  const actualEndMs =
    chunkIndex < totalChunks - 1 ? chunkEndMs + overlapMs : chunkEndMs;
  ...
  const limitedData =
    data.length > maxCandlesPerChunk ? data.slice(0, maxCandlesPerChunk) : data;
...
export function generateChunkRange(
  ...
  const chunkDurationMs =
    (paginationConfig.chunkSizeHours || 24 * 7) * 60 * 60 * 1000;
  const totalChunks = Math.ceil(totalDurationMs / chunkDurationMs);
...
export function calculateOptimalInitialViewRange(
  backtestRange: IRange<Date>,
  maxPercentage: number = 0.05, // 5% of total range
  maxDays: number = 7, // Max 1 week
): IRange<Date> {
  const totalDurationMs =
    backtestRange.to.getTime() - backtestRange.from.getTime();
  ...
  const endTime = backtestRange.to.getTime();
  const startTime = Math.max(
    endTime - actualDurationMs,
    backtestRange.from.getTime(),
  );
