  ...
  (props, ref) => {
    ...
    const chartApiRef = useRef<ChartApiRef>({
      ...
      free(series?: ISeriesApi<any>): void {
        if (this._api && series && !this.isRemoved) {
          try {
            ...
            console.warn(
              "Failed to remove series - it may have already been removed:",
              error,
            );
          }
        } else if (!series) {
      ...
      addDataFeed(dataFeed: DataFeed<any>) {
        ...
        console.log(
          "Added new datafeed to the list, currently updating %d",
          this._dataFeeds.length,
        );
          ...
          if (chartApiRef.current._dataFeeds.length === 0) {
            ...
            chart
              ...
              .unsubscribeVisibleTimeRangeChange(
                chartApiRef.current.timeRangeChangeEventHandler,
              );
            console.log(
    ...
    useLayoutEffect(() => {
      ...
      chart
        ...
        .subscribeVisibleTimeRangeChange(
          currentRef.timeRangeChangeEventHandler,
        );
      return () => {
      ...
      if (chartApiRef.current._dataFeeds.length) {
        ...
        const eventHandler = (timeRange: IRange<Time> | null) => {
          ...
          chartApiRef.current._dataFeeds.forEach((dataFeed) => {
            ...
            if (
              dataFeed.isLoading &&
              chartApiRef.current._lastNotLoadingViewRange
            ) {
              chartApiRef.current
                .api()
                .timeScale()
                .setVisibleRange(chartApiRef.current._lastNotLoadingViewRange);
            }
        ...
        chartApiRef.current
          .api()
          .timeScale()
          .subscribeVisibleTimeRangeChange(eventHandler);
      }
    }, [chartApiRef.current._dataFeeds.length]);
