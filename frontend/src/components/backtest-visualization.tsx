...
import { backtestDatesToRange, generateSampleCandlestickData, generateSampleCandlestickDataFromTimes } from "@/utils/sample-data-generator"
      ...
      <div className="border-b bg-background p-4 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div>
              <h1 className="text-xl font-bold">{strategy.name} - {backtest.name}</h1>
              <p className="text-xs text-muted-foreground">
                Backtest ID: {backtest.id} | {backtestInfo.dateRange.start.toLocaleDateString()} - {backtestInfo.dateRange.end.toLocaleDateString()}
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-muted-foreground">
                    {Math.ceil((backtestInfo.dateRange.end.getTime() - backtestInfo.dateRange.start.getTime()) / (1000 * 60 * 60 * 24))} days
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Current View:</span>
                  <span className="text-muted-foreground text-xs">
                    {backtestInfo.dateRange.start.toLocaleDateString()} - {backtestInfo.dateRange.end.toLocaleDateString()}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Return:</span>
