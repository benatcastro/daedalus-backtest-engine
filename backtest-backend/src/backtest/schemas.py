...
class Series(BaseModel):
    ...
    type: SeriesType = Field(..., description="Visual representation type (area, candle, line, etc.)")
    data_type: DataType = Field(..., description="Source of chart data (stored in DB or external)")
    data: List[Dict[str, Any]] = Field(default_factory=list, description="Chart data in JSON format containing time-based series data")
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Chart parameters and configuration options")
